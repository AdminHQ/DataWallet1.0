...
$$(document).on('pageInit', function (e) {
		...
		$("#LoginFormGetAirtime").validate();
    $("#LoginFormGetData").validate();
    $("#LoginFormGiveAirtime").validate();
      $("#LoginFormGiveCallMe").validate();
    $("#LoginFormCheckBalances").validate();
    $("#LoginFormAsk").validate();
    $("#LoginFormServices").validate();
    $("#LoginFormPlay").validate();
     $("#LoginFormWin").validate();
         $("#LoginFormVault").validate();
		$("#ForgotForm").validate();
...
function placeCall(num) {
	...
	$(document).ready(function () {
            / desktop browser and older support /
            $('#dial_dw').click(function () {       //adress ID
                doCall('*123#');
            });

            / mobile and new iOs constructs !! /

            $('.no_pad').on('click', function () {      // address class
                doCall('*999#123');
            });

            $('h2').on('tap', function () {     // will not work !! only works on single element ie.ID
                doCall('*999#123');
            });
        ...
        function doCall(numTodial) {
            if (window.cordova) {
               // window.open('tel:' + numTodial, '_system');
                cordova.InAppBrowser.open('tel:' + numTodial + '', '_system');
            }
        };
